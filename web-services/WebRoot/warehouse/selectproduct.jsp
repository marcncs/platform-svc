	...
	function Affirm(){
		...
		for(var i=0;i<document.listform.pid.length;i++){
			if(document.listform.pid[i].checked){
				...
				arrunitid[k]=document.listform.unitid[i].value;
				...
				arrcost[k]=document.listform.cost[i].value;
				k++;
				...
				arrunitid[0]=document.listform.unitid.value;
				...
				arrcost[0]=document.listform.cost.value;
				flag=true;//只要选中一个就设为true
		}
		
		if(flag){
	/*		setCookie("productid",arrid);
			setCookie("productname",arrproductname);
			setCookie("specmode",arrspecmode);
			setCookie("unitid",arrunitid);
			setCookie("countunit",arrcountunit);
			setCookie("batch",arrbatch);
			setCookie("quantity",arrquantity);
			*/
			var product={productid:arrid.slice(0),productname:arrproductname.slice(0),specmode:arrspecmode.slice(0),unitid:arrunitid.slice(0),countunit:arrcountunit.slice(0),batch:arrbatch.slice(0),quantity:arrquantity.slice(0),cost:arrcost.slice(0)};
    ...
    <td><table width="100%" height="40" border="0" cellpadding="0" cellspacing="0" class="title-back">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
		  ...
		  <td width="11%" align="right">产品类别：</td>
            <td width="18%"><select name="KeyWordLeft" id="KeyWordLeft">
			 ...
			 <td width="14%"  align="right">品牌：</td>
            <td width="9%" >${brand}</td>
            <td width="14%"  align="right"><input name="wid" type="hidden" id="wid" value="${wid}">
            名称关键字：</td>
            <td width="34%"><input type="text" name="KeyWord" value="${KeyWord}">
          ...
          <logic:iterate id="p" name="sls" >
            <tr align="center" class="table-back" >
              ...
              <td><input type="hidden" name="productname" value="${p.psproductname}">
                ${p.psproductname}</td>
              <td><input name="specmode" type="hidden" id="specmode" value="${p.psspecmode}">
			  <input name="cost" type="hidden" id="cost" value="${p.cost}">
                ${p.psspecmode}</td>
              <td><input type="hidden" name="unitid" id="unitid${count}"  value="${p.countunit}">
                   <input type="text" name="countunit" size="6" value="${p.unitname}" onFocus="selectUnit(this,'${p.productid}','2')"></td>
              <td><input name="batch" type="hidden" id="batch" value="${p.batch}">
              ${p.batch}</td>
              <td><input name="quantity" type="hidden" id="quantity${count}" value="${p.stockpile}">
                <input type="hidden" name="stockpile" id="stockpile${count}" value="${p.stockpile}"><span id="spanstockpile${count}">${p.stockpile}</span></td>
            </tr>
      ...
      <table width="100%"  border="0" cellpadding="0" cellspacing="0">
        <tr> 
          ...
          <td width="70%" align="right"> <presentation:pagination target="/warehouse/selectProductAction.do"/>          </td>
        </tr>
      </table>
