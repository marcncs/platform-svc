...
function Check(){
	...
	function SelectOutSingleProduct(pid,pname,pmcode,outUnitId,outUnit,wasteUnit){
		...
		var p=showModalDialog("../common/selectSingleProductAction.do?wid="+wid,null,"dialogWidth:20cm;dialogHeight:17cm;center:yes;status:no;scrolling:no;");
		if(p==undefined){return;}
	...
	function SelectInSingleProduct(pid,pname,pmcode,pidin,pnameout,inUnitId,inUnitName){
		...
		var p=showModalDialog("../common/selectSingleProductAction.do?proId="+proId+"&wid="+wid,null,"dialogWidth:20cm;dialogHeight:17cm;center:yes;status:no;scrolling:no;");
		if(p==undefined){return;}
		document.getElementById(pid).value=p.id;
		document.getElementById(pname).value=p.packsizename;
		document.getElementById(pmcode).value=p.nccode;
															...
															<fieldset align="center">
																<table width="100%" border="0" cellpadding="0"
																	...
																	<input name="boxQuantity" id="boxQuantity"
																		type="hidden" value="${boxQuantity}">
																	<input name="outUnitId" id="outUnitId"
																		type="hidden" value="${outUnitId}">
																	<input name="inUnitId" id="inUnitId"
																		type="hidden" value="${inUnitId}">
																	<tr>
																		<td align="right">
																			分包前产品：
																		</td>
																		<td>
																			...
																			<input name="outProductName" type="text"
																				id="outProductName" size="20" value="${outProductName}"
...
<%--																			<span id="outMCode"></span>--%>
																			<input type="text" id="outMCode" name="outMCode" value="${outMCode}" size="8"
																		...
																		<td>
																			<input type="text" id="outBatch" name="outBatch" value="${outBatch}" size="10"
																				maxlength="30" />
																		</td>
																		<td align="right">
																			可用库存:
																		...
																		<td>
																			<input type="hidden" id="stockpile" name="stockpile" value="${stockpile}"
																				maxlength="30" />
																			<span id="stockpileSpan">${stockpile}</span><span id="stockpileUnitName"><windrp:getname key='CountUnit' value='${outUnitId}' p='d' /></span>
																		</td>
																		<td align="right">
																			分包前数量:
																		</td>
																		<td>
																			<input type="text" id="outQuantity"
																				...
																				value="<windrp:format value="${outQuantity}" p="##0.000" />"
																				dataType="Require" msg="必须录入分包前数量!">
																			<span id="outUnitName"><windrp:getname key='CountUnit' value='${outUnitId}' p='d' /></span><span class="style1">*</span>
																		</td>
																	</tr>
																	<tr>
																		<td align="right">
																		...
																		<td>
																			<input name="inProductId" type="hidden"
																				id="inProductId" value="${inProductId}">
																			<input name="inProductName" type="text"
																				dataType="Require" msg="必须录入分包后产品!"
																				id="inProductName" size="20" value="${inProductName}"
																				readonly>
																			<a
...
<%--																			<span id="inMCode"></span>--%>
																			<input type="text" id="inMCode" name="inMCode" value="${inMCode}" size="8"
																				maxlength="30" readonly/> 
																		...
																		<td>
																			<input type="text" id="inBatch" name="inBatch" value="${inBatch}" size="10"
																				maxlength="30" />
																		</td>
																		<td align="right">
																			每件数量:
																		...
																		<td>
																			<input type="hidden" id="defaultUnitQty" name="defaultUnitQty" value="${defultUnitQty}"
																				maxlength="30" />
																			<span id="defaultUnitQtySpan">${defultUnitQty}</span><span id="inCountUnitName"><windrp:getname key='CountUnit' value='${inCountUnitId}' p='d' /></span>
																		</td>
																		<td align="right">
																			分包后数量:
																		</td>
																		<td>
																			<input type="text" id="inQuantity" name="inQuantity" value="<windrp:format value="${inQuantity}" p="##0.000" />"
																				onkemaxlength="20" onkeyup="checkInt(this)"
																				onblur="checkInValue(this)"
																				dataType="Require" msg="必须录入分包后数量!">
																			<span id="inUnitName"><windrp:getname key='CountUnit' value='2' p='d' /></span><span class="style1">*</span>
																		</td>
																	</tr>
																	<tr>
																		...
																		<td>
																			<input type="text" id="wasteQuantity" value="<windrp:format value="${wasteQuantity}" p="##0.000" />"
																				name="wasteQuantity" maxlength="20" readonly>
																			<span id="wasteUnitName"><windrp:getname key='CountUnit' value='${outUnitId}' p='d' /></span>
																		</td>
																	</tr>
																</table>
															</fieldset>
