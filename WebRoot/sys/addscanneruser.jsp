...
<script language=javascript>
	function CheckInput(){
		var scannerid =document.getElementById("scanner").value.trim();
    ...
    <td><table width="100%" height="40" border="0" cellpadding="0" cellspacing="0" class="title-back">
        <tr> 
          ...
          <c:if test="${type=='ADD'}">
           ...
           <c:if test="${type=='EDIT'}">
          	<td width="772"> 修改采集器配置</td>
          </c:if>
        </tr>
      </table>
	  <form name="referform" method="post" action="../sys/addScannerUserAction.do" onSubmit="return CheckInput();">
	  ...
	  <tr>
	     <td width="10%" height="20" align="right">用户名称：</td>
          <td width="60%">${uname }</td>
          <c:if test="${type=='ADD'}">
	          <td width="10%" align="right">采集器编号：</td>
	          <td width="40%"><input name="scanner" type="text" id="scanner"><span class="text-red3">*</span></td>
	           <td><input name= "type" type="hidden" value="ADD"></td>
          </c:if>
           <c:if test="${type=='EDIT'}">
	          <td width="10%" align="right">采集器编号：</td>
	          <td width="40%"><input name="scanner" type="text" value="${su.scanner}"><span class="text-red3">*</span></td>
	          <td><input name= "type" type="hidden" value="EDIT"></td>
	           <td><input name= "suid" type="hidden" value="${su.id }"></td>
          </c:if>
