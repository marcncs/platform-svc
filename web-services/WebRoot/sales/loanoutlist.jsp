...
function addNew(){
	window.open("toAddLoanOutAction.do","","height=650,width=1000,top=50,left=20,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no");
	}

	function Update(){
		if(checkid!=""){
		window.open("toUpdLoanOutAction.do?ID="+checkid,"","height=650,width=1000,top=50,left=20,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no");
		}else{
		alert("请选择你要操作的记录!");
		}
	}
	...
	function Del(){
		...
		if(window.confirm("您确认要删除所选的记录吗？如果删除将永远不能恢复!")){
			window.open("../sales/delLoanOutAction.do?SOID="+checkid,"newwindow","height=250,width=500,top=250,left=250,toolbar=no,menubar=no,scrollbars=auto,resizable=no,location=no,status=no");
			}
		}else{
		alert("请选择你要操作的记录!");
		}
	...
	function OrderDetail(){
		if(checkid!=""){
		document.all.submsg.src="../sales/loanOutDetailAction.do?ID="+checkid;
		}else{
		alert("请选择你要操作的记录!");
		}
	}
	...
	document.search.UName.value=getCookie("cname");
}
</script>
...
<table width="100%" border="1" cellpadding="0" cellspacing="1" bordercolor="#BFC0C1">
 ...
 <form name="search" method="post" action="listLoanOutAction.do">
      <table width="100%"  border="0" cellpadding="0" cellspacing="0">
       
          <tr class="table-back"> 
            <td width="9%"  align="right">借货人：</td>
            <td width="20%"><select name="UID">
            ...
            <td width="9%" align="right">是否回收： </td>
            <td width="18%">${isreceiveselect}</td>
          </tr>
          <tr class="table-back">
            <td  align="right">是否转零售： </td>
            <td>              ${istranssaleselect} </td>
  ...
  <input type="text" name="EndDate" onFocus="javascript:selectDate(this)"></td>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="1">
          ...
          <tr class="table-back" onClick="CheckedObj(this,'${s.id}');"> 
            <td >${s.id}</td>
            <td>${s.uname} <a href="../sales/listCustomerAction.do?CID=${s.uid}"><img src="../images/CN/go.gif" width="10" height="10" border="0"></a></td>            
            <td>${s.consignmentdate}</td>
            <td>${s.totalsum}</td>
            <td>${s.isauditname}</td>
            <td>${s.isreceivename}</td>
			<td>${s.makeidname} </td>
			<td>${s.makedate}</td>            
          </tr>
          </logic:iterate>
  ...
  <tr>
    <td width="48%"><table  border="0" cellpadding="0" cellspacing="0">
          ...
          <tr> 
			...
			<presentation:pagination target="/sales/listLoanOutAction.do"/>	
            </td>
          </tr>
