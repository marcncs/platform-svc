...
<section>
    ...
    <div class="contentpanel main-body">
      <div class="panel panel-default">
        <div class="panel-heading">
          <h4 class="panel-title">编辑企业故事</h4>
        </div>
        <!-- panel-body -->
        <div class="panel-body form-horizontal">
        	<div class="form-group">
                ...
                <button class="btn btn-sm btn-info" id="btn_update_news_file_upload">本地上传</button>
                <button class="btn btn-sm btn-info" onclick="chooseImgFromMedias()">从图片库选择</button>
                <input id="txt_update_news_pic_url" type="hidden"/>
                <div id="img_update_news_container" style="padding-top: 15px; display:none;">
                	<img id="img_update_news" width="200px">
                </div>
                <div id="progress_bar"></div>
              </div>
              <div style="display:none;" class="checkbox block"><label><input id="chk_update_news_pic_is_show" type="checkbox" checked> 封面图片显示在正文中</label></div>
						</div>
					</div>
					<div class="form-group">
        ...
        <div class="fixed-footer">
					<button id="btn_submit_update_news" class="btn btn-primary">确 定</button>&nbsp;
					<button id="btn_close_update_news" class="btn btn-default">关 闭</button>
			  </div><!-- panel-footer -->
  ...
  <script src="<%=path%>/res/custom/wz-media.js"></script>
	...
	<script type="text/javascript">
		...
		$().ready(function() {
	      ...
	      zIndex : 1030
			...
			ue.ready(function() {
	      //获取产品信息
    		$.ajax({url: "<%=path %>/qr/mfr_admin/news/<%=newsId %>", type: "GET", dataType: "json", async: false, 
    			success: function(result) {
    				...
    				if (manufacturernews) {
    					...
    					$("#img_update_news_container").show();
    					$("#img_update_news").attr("src", manufacturernews.picUrl);
    					$("#txt_update_news_pic_url").attr("value", manufacturernews.picUrl);
    					$("#chk_update_news_pic_is_show").attr("checked", manufacturernews.isShowPic);
    				}
    			}
    		});
	    });
			...
			//初始化图片上传
			imgUploader("btn_update_news_file_upload", "progress_bar", 5, 
				function(data) {
					//加入到素材库中
					try {
					...
					$("#txt_update_news_pic_url").attr("value", data.url);
					$("#img_update_news_container").show();
					$("#img_update_news").attr("src", data.url);
				}, 
				function(err, errTip) {
					if (err.code == -600) {
						$("#progress_bar").next('label').eq(0).remove();
						$("#progress_bar").after('<label for="name" class="error">故事封面图大小不能超过5M</label>');
					}
				}
			);
			
			$("#txt_update_news_pic_url").change(function(){
				$("#chk_update_news_pic_is_show").next('label').eq(0).remove();
			});
      ...
      $("#btn_submit_update_news").click(function(e) {
      	...
      	var picUrl = $("#txt_update_news_pic_url").val();
    		var isShowPic = $("#chk_update_news_pic_is_show").is(':checked');
    		
      	$("#txt_update_news_pic_url").next('label').eq(0).remove();
      	$("#div_update_news_title").next('label').eq(0).remove();
      	$("#div_update_news_summary > label:last").remove();
      	$("#ueditor_update_news_content").next('label').eq(0).remove();
      	if (picUrl == null || $.trim(picUrl) == '') {
					$("#txt_update_news_pic_url").after('<label for="name" class="error">请选择封面图片</label>');
					$("body").scrollTo(".panel-body", 500);
					return ;
      	}
      	if (title == null || $.trim(title) == '' || $.trim(title).length > news_title_max_len) {
      		$("#div_update_news_title").after('<label for="name" class="error">故事标题不能为空且长度不能超过' + news_title_max_len + '字</label>');
      		$("body").scrollTo(".panel-body", 500);
      		return ;
      	}
      	...
      	if (content == null || $.trim(content) == '') {
      		$("#ueditor_update_news_content").after('<label for="name" class="error">请输入故事内容</label>');
      		return ;
      	}
      	
      	var data = {pic_url : picUrl, title : title, summary : summary, content : content, is_show_pic : isShowPic};
      	$.ajax({url: "<%=path %>/qr/mfr_admin/news/<%=newsId %>", type: "POST", dataType:"json", data: data,
					success: function(d) {
			...
			$("#btn_close_update_news").on("click", function(e) {
				window.opener = null; 
				window.open('', '_self'); 
				window.close() 
			});
		...
		function chooseImgFromMedias() {
			showMediaImageChooser(1, function(data) {
	   		if (data) {
	   			$("#txt_update_news_pic_url").attr("value", data[0].url);
					$("#img_update_news").attr("src", data[0].url);
					$("#img_update_news_container").show();
	   		}
    	});
		}
	...
	<!-- 图片选择器 -->
	<jsp:include page="media_image_chooser.jsp" flush="true" />
</body>
</html>
