...
<%@page import="com.winsafe.drp.dao.MediaImageGroup"%>
<%@page import="com.winsafe.drp.dao.MediaImage"%>
            ...
            <div class="row filemanager">
                <div class="panel-heading">
                	<c:if test="${group != null}">
                		...
                		<c:if test="${group.getId() != 0}">
                			&nbsp;&nbsp;&nbsp;&nbsp;
		               		<a href="javascript:displayUpdateGroupModal(${group.id});">重命名</a>
		               		&nbsp;&nbsp;&nbsp;&nbsp;
		               		<a href="javascript:displayDeleteGroupModal(${group.id})">删除分组</a>
                		</c:if>
                	</c:if>
                ...
                <div class="contentpanel banner-list">
                		...
                		<div class="col-xs-6 col-sm-4 col-md-3 image">
			              <div class="thmb">
			                ...
			                <div class="checkbox block">
			                	<label><input type="checkbox" name="chb_image" data-id="${image.id}" onclick="setSelectAll();">
			                	<span id="image_title_${image.id}" class="fm-title title-display">${image.title}</span></label>
			                </div>
			                <ul class="blog-meta">
				                <li><a href="javascript:displayUpdateImageModal(${image.id})">编辑</a></li>
				                <li><a href="javascript:displayMoveImageModal(${image.id})">移动</a></li>
				                <li><a href="javascript:displayDeleteImageModal(${image.id})">删除</a></li>
				              </ul>
			              </div><!-- thmb -->
			            </div>
                	</logic:iterate>
                	...
                	%>
                			<div class="col-xs-6 col-sm-4 col-md-3 image">
					              <div class="thmb">
					                ...
					                <div class="checkbox block">
					                	<label><input type="checkbox" name="chb_image" data-id="${image.id}" onclick="setSelectAll();">
					                	<span id="image_title_${image.id}" class="fm-title title-display"><%=image.getTitle() %></span></label>
					                </div>
					                <ul class="blog-meta">
						                <li><a href="javascript:displayUpdateImageModal(${image.id})">编辑</a></li>
						                <li><a href="javascript:displayMoveImageModal(${image.id})">移动</a></li>
                  ...
                  <jsp:include page="include_pageinfo.jsp" flush="true" />
	...
	<script>
		...
		function successCallback(data) {
			try {
				...
				wzMedia.addImage(title, data.url, data.fsize, '${group.id}');
			} catch (e) {
		...
		jQuery(document).ready(function() {
			...
			imgUploader("btn_image_file_upload", "progress_bar", 2, successCallback, failCallback, true, 
				function(){
					window.location.href = "<%=path%>/qr/mfr_admin/media/images?group_id=${group.id}";
				}
			);
