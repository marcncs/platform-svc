...
function addNew(){
	window.open("toAddRetailAction.do","","height=650,width=1000,top=50,left=20,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no");
	}

	function Update(){
		if(checkid!=""){
		window.open("toUpdRetailAction.do?ID="+checkid,"","height=650,width=1000,top=50,left=20,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no");
		}else{
		alert("请选择你要操作的记录!");
		}
	}
...
<form name="search" method="post" action="listRetailAction.do">
  ...
  <tr>
    <td width="48%"><table  border="0" cellpadding="0" cellspacing="0">
          ...
          <tr> 
			...
			<presentation:pagination target="/sales/listRetailAction.do"/>	
            </td>
          </tr>
