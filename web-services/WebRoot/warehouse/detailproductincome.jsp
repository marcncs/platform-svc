...
<script language="javascript">
	...
	function toaddidcode(billid,bdid,wid,isaudit,batch){
			popWin("../warehouse/toAddProductIncomeIdcodeiiAction.do?billid="+billid+"&bdid="+bdid+"&wid="+wid+"&isaudit="+isaudit+"&batch="+batch,1000,650);
	}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#BFC0C1">
  <tr>
	...
	<c:if test="${type == '2'}">
      		<tr> <td width="976"> 产成品入库单详情 </td></tr>
      	</c:if>
      	<c:if test="${type != '2'}">
      		<tr>
			    <td width="10"><img src="../images/CN/spc.gif" width="10" height="8"></td>
			          <td width="976"> 产成品入库单详情 </td>
			          <c:if test="${type!=2}">
			          <td width="257" align="right"><table width="120"  border="0" cellpadding="0" cellspacing="0">
			            <tr>
			              <td width="60" align="center"><a href="javascript:ProductIncome('${pif.id}');">打印</a></td>
...
<%--          <td width="4%">成本金额</td>--%>
        ...
        <tr class="table-back-colorbar">
          ...
          <td>${p.specmode}</td>
          <td>${p.batch}</td>
          <td><windrp:idcodebit value="${p.productid}" idcodeclick="toaddidcode('${pif.id}','${p.id}','${pif.warehouseid}','${pif.isaudit}','${p.batch}')" bitclick="toaddbit('${pif.id}','${p.id}','${pif.warehouseid}','${pif.isaudit}')"/></td>
          <td><windrp:format value="${p.quantity}"/></td>
          <td><windrp:getname key='CountUnit' value='${p.unitid}' p='d'/></td>
<%--          <td><windrp:format value="${p.quantity}"/></td>--%>
<%--          <td><windrp:format value="${p.confirmQuantity}"/></td>--%>
