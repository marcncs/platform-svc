...
public class ExcPutOrganTradesOrganTotalAction extends BaseAction{
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try{
	        ...
	        List list = asod.getOrganTradesOrganTotal(whereSql);
			...
			response.setHeader("content-disposition",
					"attachment; filename=OrganTradesOrganTotal.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出渠道换货按机构汇总");
		}catch(Exception e){
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "渠道换货按机构汇总",wchT));
			...
			sheets[j].addCell(new Label(0, start+4, "供货机构编号", wcfFC));
			sheets[j].addCell(new Label(1, start+4, "供货机构", wcfFC));
			sheets[j].addCell(new Label(2, start+4, "换货机构编号", wcfFC));
			sheets[j].addCell(new Label(3, start+4, "换货机构", wcfFC));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Label(1, row, getOrganName(p.get("porganid"))));
				sheets[j].addCell(new Label(2, row, p.get("makeorganid").toString()));
