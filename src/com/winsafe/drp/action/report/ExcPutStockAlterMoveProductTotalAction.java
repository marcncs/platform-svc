...
public class ExcPutStockAlterMoveProductTotalAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出订购按产品汇总");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "渠道订购按产品汇总",wchT));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Number(4, row, Double.valueOf(p.get("unitprice").toString()),wcfN));
