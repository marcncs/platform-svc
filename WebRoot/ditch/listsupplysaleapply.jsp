...
<html>
	...
	function CheckedObj(obj,objid){
	 ...
	 obj.className="event";
	 checkid=objid;
	 SupplySaleApplyDetail();
	}
	
	function addNew(){
		popWin("../ditch/toAddSupplySaleApplyAction.do",1000,650);
	}

	function Update(){
		if(checkid!=""){
			popWin("../ditch/toUpdSupplySaleApplyAction.do?ID="+checkid,1000,650);
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	...
	function Inquire(){
		if(checkid!=""){
				location.href("../ditch/detailSupplySaleApplyAction.do?ID="+checkid);
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	
	function SupplySaleApplyDetail(){
		if(checkid!=""){
			document.all.submsg.src="../ditch/detailSupplySaleApplyAction.do?ID="+checkid+"&ISAUDIT=${ISAUDIT}";
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	...
	function Del(){
		if(checkid!=""){
			if(window.confirm("您确认要删除编号为："+checkid+" 的代销申请吗？如果删除将永远不能恢复!")){
				popWin("../ditch/delSupplySaleApplyAction.do?ID="+checkid,500,250);
			}
		}else{
			alert("请选择你要操作的记录!");
		}
	...
	function excput(){
		search.target="";
		search.action="../ditch/excPutSupplySaleApplyAction.do?ISAUDIT=${ISAUDIT}";
		search.submit();
		search.action="listSupplySaleApplyAction.do";
	}
	function print(){
		search.target="_blank";
		search.action="../ditch/printListSupplySaleApplyAction.do?ISAUDIT=${ISAUDIT}";
		search.submit();
		search.target="";
		search.action="listSupplySaleApplyAction.do";
	}
	...
	function oncheck(){
		if('${ISAUDIT}'=='no'){
			...
			search.action="listAuditSupplySaleApplyAction.do";
			search.submit();
		}
	}
</script>
			...
			<tr>
				<td>
					...
					<form name="search" method="post"
								action="listSupplySaleApplyAction.do" onSubmit="return oncheck();">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							...
							<c:if test="${ISAUDIT=='no'}">
								...
								<td class="SeparatePage">
								<c:choose>
						          <c:when test="${ISAUDIT=='no'}"><pages:pager action="../ditch/listSupplySaleApplyAction.do" /></c:when>
						          <c:otherwise><pages:pager action="../ditch/listAuditSupplySaleApplyAction.do" /></c:otherwise>
						        </c:choose>	
								</td>
							</tr>
						</table>
					...
					<div id="abc" style="overflow-y: auto; height: 600px;">
						...
						<br />
							...
							<div style="width:100%">
					      	<div id="tabs1">
					            <ul>
					              <li><a href="javascript:SupplySaleApplyDetail();"><span>代销申请详情</span></a></li>
					            </ul>
					          </div>
