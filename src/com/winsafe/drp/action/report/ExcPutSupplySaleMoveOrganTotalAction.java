...
public class ExcPutSupplySaleMoveOrganTotalAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			List list = aso.getSupplySaleMoveOrganTotal(whereSql);
			...
			response.setHeader("content-disposition",
					"attachment; filename=SupplySaleMoveOrganTotal.xls");
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "渠道代销按机构汇总",wchT));
			...
			sheets[j].addCell(new Label(0, start+4, "制单机构编号",wcfFC));
			sheets[j].addCell(new Label(1, start+4, "制单机构名称",wcfFC));
			sheets[j].addCell(new Label(2, start+4, "申请机构编号",wcfFC));
			sheets[j].addCell(new Label(3, start+4, "申请机构名称",wcfFC));
			sheets[j].addCell(new Label(4, start+4, "调入机构编号",wcfFC));
			sheets[j].addCell(new Label(5, start+4, "调入机构名称",wcfFC));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Label(2, row, p.get("supplyorganid").toString()));
				sheets[j].addCell(new Label(3, row, organ.getOrganName(p.get("supplyorganid").toString())));
				sheets[j].addCell(new Label(4, row, p.get("inorganid").toString()));
				sheets[j].addCell(new Label(5, row, organ.getOrganName(p.get("inorganid").toString())));
