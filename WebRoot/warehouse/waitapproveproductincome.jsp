...
var actid=0;
	...
	function Approve(){
		if(checkid!=""){	
			window.open("../warehouse/toApproveProductIncomeAction.do?ID="+checkid+"&actid="+actid,"","height=650,width=1000,top=50,left=20,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no");
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	...
	function CancelApprove(){
		if(checkid!=""){
			window.open("../warehouse/cancelApproveProductIncomeAction.do?PIID="+checkid+"&actid="+actid,"newwindow","height=250,width=500,top=250,left=250,toolbar=no,menubar=no,scrollbars=auto,resizable=no,location=no,status=no");
		}else{
		alert("请选择你要操作的记录!");
		}
	}
...
<input type="text" name="EndDate" onFocus="javascript:selectDate(this)">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="1">
          ...
          <tr class="table-back" onClick="CheckedObj(this,'${pi.id}','${pi.actid}');"> 
            <td  ><a href="../warehouse/productIncomeDetailAction.do?ID=${pi.id}">${pi.id}</a></td>
            <td>${pi.warehouseidname}</td>
            <td>${pi.producebatch}</td>
