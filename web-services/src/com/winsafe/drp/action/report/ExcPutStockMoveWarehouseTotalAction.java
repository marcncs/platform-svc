...
public class ExcPutStockMoveWarehouseTotalAction extends BaseAction{
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try{
			...
			AppStockMove asod=new AppStockMove(); 
	        List list = asod.getStockMoveWarehouseTotal(whereSql);
			...
			response.setHeader("content-disposition",
					"attachment; filename=StockMoveWarehouseTotal.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出转仓按仓库汇总");
		}catch(Exception e){
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "转仓按仓库汇总",wchT));
