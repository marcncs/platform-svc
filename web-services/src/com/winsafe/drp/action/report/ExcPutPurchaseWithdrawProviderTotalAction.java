...
public class ExcPutPurchaseWithdrawProviderTotalAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			response.setHeader("content-disposition",
					"attachment; filename=PurchaseWithdrawProviderTotal.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出采购退货按供应商汇总");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os, HttpServletRequest request)
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "采购退货按供应商汇总",wchT));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Label(0, row, p.get("pid").toString()));
				sheets[j].addCell(new Label(1, row, p.get("pname").toString()));
