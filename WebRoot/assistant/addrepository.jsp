...
var allid=0;
	...
	var id = $F('psid');
	//alert(vID);
        var url = 'ajaxValidateProductIDAction.do?ID='+id;
        var pars = '';
  ...
  <tr>
    <td><table width="100%" height="40" border="0" cellpadding="0" cellspacing="0" class="title-back">
      <tr>
        ...
        <td width="772"> 新增知识库</td>
      </tr>
    </table>
        <form action="addRepositoryAction.do" method="post" enctype="multipart/form-data" name="repositoryForm" onSubmit="return formChk();">
              ...
              <td width="20%">
                <select name="rtid" id="rtid">
                  <logic:iterate id="ps" name="uls" >
                    <option value="${ps.structcode}" <c:if test="${ps.structcode==psid}"><c:out value="selected"/></c:if>>
                      <c:forEach var="i" begin="1" end="${fn:length(ps.structcode)/2}" step="1">
                        <c:out value="--"/></c:forEach>${ps.sortname}</option>
                  </logic:iterate>
              </select></td>
              <td width="10%" align="right">标题：</td>
              <td width="23%"><input name="title" type="text" id="title" size="38">
              <span class="style1">*</span></td>
             
            </tr>
			 <tr>
			   <td  align="right">内容：</td>
			   <td colspan="3"><textarea name="content"  id="content" style="width:620; height:400;"></textarea></td>
