...
public class ExcPutPurchaseTradesProductTotalAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			String blur = getKeyWordCondition("ProductName",
					"ProductID","SpecMode","MakeDate", "MakeOrganID");
			...
			response.setHeader("content-disposition",
					"attachment; filename=PurchaseTradesProductTotal.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出采购换货按产品汇总");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "采购换货按产品汇总",wchT));
