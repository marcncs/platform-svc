...
public class StockAlterMoveImportMatchService {
	...
	// 默认构造器
	public StockAlterMoveImportMatchService() {
	}
...
//				if (!"2768".equals(styleNo)) {
//					errorMsg.append("单据信息错误，配货出库单样式编号有误<br>");
//					throw new Exception("单据信息错误，配货出库单样式编号有误");
//				}
				...
				// 查找数据库中的记录
				 ...
				 if (sm == null) { // 新增订购单
						...
						sm.setTallyid(null);
						sm.setReceiveorganid(receiveOrganid);
						sm.setReceiveorganidname(null);
						sm.setNccode(nccode);
						sm.setKeyscontent(null);
						asm.saveStockAlterMove(sm);
					} else {
		...
		String t="总数量："+(rowCount-49)+"，成功数量："+successCount+"，失败数量："+(rowCount-49-successCount)+"<br>";
