...
import com.winsafe.hbm.util.pager.DeptSelectTagHandle;
import com.winsafe.drp.dao.Dept;
import com.winsafe.drp.dao.UsersBean;
import com.winsafe.drp.server.DeptService;


public class DeptSelectTagHandle extends TagSupport {
	protected Logger logger=Logger.getLogger(DeptSelectTagHandle.class);
	...
	private DeptService os = new DeptService();
	...
	protected void outPagination()throws JspException{
		...
		try{				
			if ( name.equals("") ){
				throw new JspException("<data:dept tag the name parameter is null");
			}
			if ( value == null ){
				value = "";
			}else{
			...
			List<Dept> dlist = os.getDeptByOID(usersBean.getMakeorganid());
			StringBuffer sb = new StringBuffer();
			for ( Dept dept : dlist ){				
				if ( dept.getId().toString().equalsIgnoreCase(value) ){				
					sb.append("<option value=\""+dept.getId()+"\" selected>").append(dept.getDeptname()).append("</option>");
				}else{
					sb.append("<option value=\""+dept.getId()+"\">").append(dept.getDeptname()).append("</option>");
				}			
			}
			...
			throw new JspException("<data:dept tag  error"+exc.getMessage());
		}
	}
