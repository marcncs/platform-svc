...
public class AddOrganWithdrawIdcodeiAction extends BaseAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		super.initdata(request);try{
			...
			OrganWithdrawIdcode pi = null;
			app.delOrganWithdrawIdcodeByPid(productid, billid, pbatch);
			for ( int i=0; i<warehousebit.length; i++ ){
				pi = new OrganWithdrawIdcode();
				pi.setId(Long.valueOf(MakeCode.getExcIDByRandomTableName("organ_withdraw_idcode", 0, "")));
				pi.setOwid(billid);
				...
				app.addOrganWithdrawIdcode(pi);
			}
			 request.setAttribute("result", "databases.add.success");
			 UsersBean users = UserManager.getUser(request);
			 DBUserLog.addUserLog(users.getUserid(),7, "入库>>渠道退货签收>>分配仓位,产品编号:"+productid);
