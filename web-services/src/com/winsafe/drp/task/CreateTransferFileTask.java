 ...
 * 
 */
public class CreateTransferFileTask {

	private static Logger logger = Logger
			.getLogger(CreateTransferFileTask.class);
	// 同步锁
...
//		transferLogs = appTl.getAllUnTransferedLogs();
	...
	private void startCreateTransferFile() throws Exception {
		//初始化库存数据
		FileTransferService invFts = FileTransferService.getTransferService(FileType.INVENTORY);
		invFts.createTransferFile();
		for(FileType fileType : FileType.values()) {
			try {
				if(fileType != FileType.INVENTORY && fileType != FileType.INVENTORY_DETAIL && fileType != FileType.CSSI_FILE) {
	...
	public static void main(String[] args) {
		new CreateTransferFileTask().run();
	}
}
