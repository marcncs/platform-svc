...
<script language="javascript">
	...
	var arrgoodsfund=new Array();
	var arrscot=new Array();
	...
	function Affirm(){
		...
		for(var i=0;i<document.listform.pid.length;i++){
			if(document.listform.pid[i].checked){
				...
				arrrid[k]=document.listform.rid[i].value;
				...
				arrcountunit[k]=document.listform.unitidname[i].value;
				...
				arrgoodsfund[k]=document.listform.goodsfund[i].value;
				arrscot[k]=document.listform.scot[i].value;
				k++;
				...
				arrrid[0]=document.listform.rid.value;
				...
				arrcountunit[0]=document.listform.unitidname.value;
				...
				arrgoodsfund[0]=document.listform.goodsfund.value;
				arrscot[0]=document.listform.scot.value;
				flag=true;//只要选中一个就设为true
		}
		
		if(flag){		
			var p={rid:arrrid.slice(0),productid:arrid.slice(0),productname:arrpordocutname.slice(0),specmode:arrspecmode.slice(0),unitid:unitid.slice(0),countunit:arrcountunit.slice(0),batch:arrbatch.slice(0),billno:arrbillno.slice(0),quantity:arrquantity.slice(0),goodsfund:arrgoodsfund.slice(0),scot:arrscot.slice(0)};
  ...
  <tr>
    <td><table width="100%" height="40" border="0" cellpadding="0" cellspacing="0" class="title-back">
        <tr> 
          ...
          <td width="772"> 选择应付款详情</td>
	    
        </tr>
      </table>
      <form name="search" method="post" action="../finance/selectPayableDetailAction.do">
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        
          <tr class="table-back"> 
		  	<td width="11%" align="right">名称关键字：</td>
            <td width="19%"><input type="text" name="KeyWord" value="${KeyWord}"></td>
            <td width="13%"  align="right">规格：</td>
            <td width="57%" ><input type="text" name="KeyWordLeft"><input type="submit" name="Submit" value="查询"></td>
          </tr>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="1">
		  ...
		  <tr class="table-back" > 
            ...
            <td >${p.productid}</td>
            <td><input type="hidden" name="rid" value="${p.pid}" ><input type="hidden" name="productname" value="${p.productname}">${p.productname}</td>
            <td><input type="hidden" name="specmode" value="${p.specmode}">${p.specmode}</td>
            <td><input type="hidden" name="unitid" value="${p.unitid}">
              <input type="hidden" name="unitidname" value="${p.unitidname}">
              ${p.unitidname}</td>
			<td><input type="hidden" name="batch" value="${p.batch}">${p.batch}</td>
            ...
            <td><input type="hidden" name="quantity" value="${p.quantity}">${p.quantity}</td>
			 <td><input type="hidden" name="goodsfund" value="${p.goodsfund}">${p.goodsfund}</td>
			  <td><input type="hidden" name="scot" value="${p.scot}">${p.scot}</td>
          </tr>
          </logic:iterate> 
      ...
      <table width="100%"  border="0" cellpadding="0" cellspacing="0">
        <tr> 
          ...
          <td width="70%"> <presentation:pagination target="/finance/selectPayableDetailAction.do"/> 
          </td>
        </tr>
      </table>
