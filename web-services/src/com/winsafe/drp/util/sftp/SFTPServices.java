...
public class SFTPServices {
	...
	private static Logger logger = Logger.getLogger(SFTPServices.class);
	...
	private ChannelSftp getChannelSftp() throws Exception {
		if(chSftp == null) {
	        ...
	        // 设置主机ip，端口，用户名，密码
			String pwd = Encrypt.getSecret(SysConfig.getSysConfig().getProperty("sftp_password"), 2);
	        sftpDetails.put(SFTPConstants.SFTP_REQ_HOST, SysConfig.getSysConfig().getProperty("sftp_host"));
	        sftpDetails.put(SFTPConstants.SFTP_REQ_USERNAME, SysConfig.getSysConfig().getProperty("sftp_username"));
	        sftpDetails.put(SFTPConstants.SFTP_REQ_PASSWORD, pwd);
	        sftpDetails.put(SFTPConstants.SFTP_REQ_PORT, SysConfig.getSysConfig().getProperty("sftp_port"));
	        //获取代理配置
