		...
		<script type="text/javascript">
			...
			function openDownFile(clientname,clientcode,date,productname){
				if(clientname != "" && clientcode != "" && date != "" && productname != ""){
					popWin("../sys/winsafeFwmMarubiDownAction.do?clientname="+encodeURIComponent(encodeURIComponent(clientname))+"&clientcode="+clientcode+"&date="+date+"&productname="+encodeURIComponent(encodeURIComponent(productname)),610,200);
				}else{
			...
			<tr>
				<td>
					<div id="bodydiv">
						...
						<form name="search" method="post" action="../sys/toDownloadMarubiBarcodeLogging.do"
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
				          <tr class="title-func-back">
				       		<td class="SeparatePage">
								<pages:pager action="../sys/toDownloadMarubiBarcodeLogging.do"/>
							</td>
				          </tr>
				        </table>
					</div>
					          ...
					          <logic:iterate id="c" name="fwmList">
						          <tr class="table-back-colorbar" onClick="CheckedObj(this);"> 
						           <td align="center">${c.fwmCodeFile}</td>
						            ...
						            <td align="center">
						            	<c:if test="${c.isApprove eq 0 || c.isDeal eq 1 || c.isDeal eq 0}">
						            		<img src="../images/CN/close.gif" border="0" width="12" height="12"/>
						            	</c:if>
						            	<c:if test="${c.isApprove eq 1 && c.isDeal eq 2}">
						            		<a href="javascript:openDownFile('${c.fwmClientName}','${c.fwmClientCode}','${c.fwmCreateDate}','${c.fwmProductName}');">导出</a>
						            	</c:if>
						            </td>
						            </tr>
					          </logic:iterate>
