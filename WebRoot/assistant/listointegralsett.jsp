...
function addNew(){
	window.open("../assistant/toAddOIntegralSettAction.do","","height=650,width=900,top=90,left=90,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=yes");
	}

	function Update(){
		if(checkid!=""){
			window.open("../assistant/toUpdOIntegralSettAction.do?id="+checkid,"","height=650,width=900,top=90,left=90,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=no");
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	...
	function Del(){
		if(checkid!=""){
			...
			window.open("../assistant/delOIntegralSettAction.do?id="+checkid,"newwindow","height=250,width=500,top=250,left=250,toolbar=no,menubar=no,scrollbars=auto,resizable=no,location=no,status=no");
			}
		}else{
			alert("请选择你要操作的记录!");
		}
	...
	setCookie("PdCookie","0");
		if(checkid!=""){
			document.all.submsg.src="oIntegralSettDetailAction.do?id="+checkid;
		}else{
			alert("请选择你要操作的记录!");
		}
	}
...
<table width="101%" border="1" cellpadding="0" cellspacing="2" bordercolor="#BFC0C1">
  <tr>
    <td><table width="100%" height="40" border="0" cellpadding="0" cellspacing="0" class="title-back">
      ...
      <form name="search" method="post" action="listOIntegralSettAction.do">
	  <table width="100%"  border="0" cellpadding="0" cellspacing="0">
          ...
          <tr class="table-back"> 
            <td width="9%"  align="right">结算机构：</td>
            <td width="19%"><select name="OID" id="OID">
              <option value="" selected>所有机构</option>
              <logic:iterate id="o" name="ols" >
                <option value="${o.id}"><c:forEach var="i" begin="1" end="${fn:length(o.id)/2}" step="1">
                  <c:out value="--"/>
            ...
            <td><input type="text" name="BeginDate" size="16" onFocus="javascript:selectDate(this)">
              -
              <input type="text" name="EndDate" size="16" onFocus="javascript:selectDate(this)"></td>
...
<tr align="center"> 
          ...
          <td width="82%"> <table  border="0" cellpadding="0" cellspacing="0" >
              <tr> 
                <td width="50%" align="right"> <presentation:pagination target="/assistant/listOIntegralSettAction.do"/> 
                </td>
              </tr>
            </table></td>
        </tr>
