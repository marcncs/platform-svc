...
<html>
	...
	function addNew(){
		popWin("../ditch/toAddPlantWithdrawAction.do",1000,650);
	}

	function Update(){
		if(checkid!=""){
			popWin("../ditch/toUpdPlantWithdrawAction.do?ID="+checkid,1000,650);
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	...
	function OrganWithdrawDetail(){
		if(checkid!=""){
			document.all.submsg.src="../ditch/detailPlantWithdrawAction.do?ID="+checkid+"&isshow=${isshow}";
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	...
	function Del(){
		if(checkid!=""){
			if(window.confirm("您确认要删除编号为："+checkid+" 的渠道退货单吗？如果删除将永远不能恢复!")){
				popWin("../ditch/delPlantWithdrawAction.do?ID="+checkid,500,250);
			}
		}else{
			alert("请选择你要操作的记录!");
		}
	...
	function excput(){
		search.target="";
		search.action="../ditch/excPutPlantWithdrawAction.do?isshow=${isshow}";
		search.submit();
		search.action="listPlantWithdrawAction.do";
	}
	function print(){
		excputform.target="_blank";
		excputform.action="../ditch/printListPlantWithdrawAction.do?isshow=${isshow}";
		excputform.submit();
	}
				...
				<td>
					<div id="div1">
						...
						<form name="search" method="post"
								action="listPlantWithdrawAction.do" onSubmit="return oncheck();">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<tr class="title-func-back">
								<ws:hasAuth operationName="/ditch/toAddPlantWithdrawAction.do">
								<td width="50">
								...
								<ws:hasAuth operationName="/ditch/toUpdPlantWithdrawAction.do">
								<td width="50">
								...
								<ws:hasAuth operationName="/ditch/delPlantWithdrawAction.do">
								<td width="50">
								...
								<ws:hasAuth operationName="/ditch/excPutPlantWithdrawAction.do">
								<td width="50">
								<a href="javascript:excput();"><img src="../images/CN/outputExcel.gif" width="16" height="16" border="0" align="absmiddle">&nbsp;导出</a>
							    </td>
							    <td width="1"><img src="../images/CN/hline.gif" width="2" height="14"></td>	
								</ws:hasAuth>
								<ws:hasAuth operationName="/ditch/printListPlantWithdrawAction.do">
								<td width="51">
								...
								<td class="SeparatePage">
						           <pages:pager action="../ditch/listPlantWithdrawAction.do" />
								</td>
							</tr>
						</table>
					</div>
...
<%--										<windrp:getname key='organ' value='${o.receiveorganid}' p='d' />--%>
      	...
      	<div id="tabs1">
            <ul>
              <li><a href="javascript:OrganWithdrawDetail();"><span>${menu }详情</span></a></li>
            </ul>
          </div>
