...
<script language="javascript">
	function Receive(smid){
			showloading();
			popWin2("../warehouse/completeOrganTradeTReceiveAction.do?ID="+smid);
	}
	
	function CancelReceive(smid){
			showloading();
			popWin2("../warehouse/cancelCompleteOrganTradeTReceiveAction.do?ID="+smid);
	}
	
	function toaddidcode(billid,bdid,wid,isaudit){
			popWin("../warehouse/toAddOrganTradesTIdcodeiiAction.do?billid="+billid+"&bdid="+bdid+"&wid="+wid+"&isaudit="+isaudit,1000,650);
	}
	function toaddbit(billid,bdid,wid,isaudit){
			popWin("../warehouse/toAddOrganTradesTIdcodeiAction.do?billid="+billid+"&bdid="+bdid+"&wid="+wid+"&isaudit="+isaudit,1000,600);
	}
	function prints(id){
			popWin("../ditch/printOrganTradesAction.do?ID="+id+"&isshow=0",900,600);
	}
</script>
      ...
      <tr>
       <td width="60" align="center"><a href="javascript:prints('${ot.id}');">打印</a></td>
        <td style="float: right; padding-right: 20px;">  
        	<c:choose>         	
	       	<c:when test="${ot.isreceive==0}"><a href="javascript:Receive('${ot.id}')">签收</a></c:when>
...
<fieldset align="center"> <legend>
	...
	<table width="100%"  border="0" cellpadding="0" cellspacing="1" class="sortable">
        ...
        <tr class="table-back-colorbar">
          ...
          <td><windrp:idcodebit value="${p.productid}" idcodeclick="toaddidcode('${ot.id}','${p.id}','${ot.outwarehouseid}','${ot.isreceive}')" bitclick="toaddbit('${ot.id}','${p.id}','${ot.outwarehouseid}','${ot.isreceive}')"/></td>           
          <td>${p.quantity}</td>
          <td>${p.canquantity}</td>
        </tr>
        </logic:iterate> 
