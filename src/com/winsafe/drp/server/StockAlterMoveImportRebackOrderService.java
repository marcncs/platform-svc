...
public class StockAlterMoveImportRebackOrderService {
	...
	public StockAlterMoveImportRebackOrderService() {
	}
	/**
	 *  文件，用户，文件名
	 * 
...
//			if (!"21".equals(styleNo)) {
//				errorMsg.append("单据信息错误，销售退货折扣单样式编号有误<br>");
//				throw new Exception("单据信息错误，销售退货折扣单样式编号有误");
//			}
		...
		String t="总数量："+(rowCount-45)+"，成功数量："+successCount+"，失败数量："+(rowCount-45-successCount)+"<br>";
		return t+"<font color='red'>"+errorMsg.toString()+"</font>";
	}
	  /**
     * 1:新增；2:更新；3:已复合
     ...
     * @throws Exception 
     */
	private void addSmd(int state, StockAlterMove sm,
			List<StockAlterMoveDetailFormBean> lBeans) throws Exception {
	 ...
	 * @throws NumberFormatException 
	 */
	private void addSmd(StockAlterMoveDetailFormBean sf, StockAlterMove sm,
			Product p) throws NumberFormatException, Exception {
