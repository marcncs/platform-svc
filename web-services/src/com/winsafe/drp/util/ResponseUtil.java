...
//		logger.debug("interface return:" + json.toString());
	...
	public static JSONObject getJsonMsg(String returnCode,String returnMsg,Object returnData, String dataName){
		JSONObject json = new JSONObject();		
		if(returnData == null){ 
			returnCode = Constants.CODE_NO_DATA;
			returnMsg = Constants.CODE_NO_DATA_MSG;
		...
		json.put("returnCode", returnCode);
		json.put("returnMsg", returnMsg);
		json.put(dataName, returnData);
		return json;
	}

	public static void writeJsonMsg(HttpServletResponse response, ResponseData responseData) throws Exception {
		JSONObject json = JSONObject.fromBean(responseData);
		write(response,json.toString(),"utf-8");
	}
}
