...
function Audit(soid){
			window.open("../assistant/auditOIntegralSettAction.do?id="+soid,"newwindow","height=250,width=500,top=250,left=250,toolbar=no,menubar=no,scrollbars=auto,resizable=no,location=no,status=no");
	}
	
	function CancelAudit(soid){
			window.open("../assistant/cancalAuditOIntegralSettAction.do?id="+soid,"newwindow","height=250,width=500,top=250,left=250,toolbar=no,menubar=no,scrollbars=auto,resizable=no,location=no,status=no");
	}
...
<a href="javascript:CancelAudit('${rf.id}')">取消复核</a></c:otherwise>
  ...
  <tr>
	...
	<fieldset align="center"> <legend>
	  ...
	  <tr>
	  	<td width="15%"  align="right">结算机构：</td>
          <td width="21%">${rf.oidname}</td>
          <td width="10%" align="right">结算积分：</td>
          <td width="21%">${rf.settintegral}</td>
          <td width="10%" align="right">换取金额：</td>
	      <td width="23%">${rf.settcash}</td>
	  </tr>
	  </table>
</fieldset>
