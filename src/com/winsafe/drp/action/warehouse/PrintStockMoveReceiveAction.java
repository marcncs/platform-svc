...
public class PrintStockMoveReceiveAction extends BaseAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		super.initdata(request);try{
			...
			parameters.put("organtitle", ino.getOrganname());
			parameters.put("title", "");
			
			String path = "";
			if (ino.getLogo().equals("")) {
				path = context.getRealPath("images\\logo.jpg");
			} else {
				path = context.getRealPath(ino.getLogo());
			}
			parameters.put("img", path);
			parameters.put("isshow", false);
			
			parameters.put("billtype", "Location Transfer Note Receive 转仓单");
