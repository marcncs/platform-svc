...
public class PrintOtherIncomeAction extends BaseAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		super.initdata(request);try{
			...
			AppOtherIncome aps = new AppOtherIncome();
			AppUsers au = new AppUsers();
			OtherIncome sb = aps.getOtherIncomeByID(id);				
			OrganService os = new OrganService();
			WarehouseService ws = new WarehouseService();
			
			
			Map parameters = new HashMap();			
			parameters.put("billtype", "盘盈清单");
			parameters.put("warehouse", ws.getWarehouseName(sb.getWarehouseid()));
			parameters.put("incomesort", HtmlSelect.getNameByOrder(request, "IncomeSort", sb.getIncomesort()));
			...
			aps.updOtherIncome(sb);

			AppOtherIncomeDetail apbd = new AppOtherIncomeDetail();
			List<OtherIncomeDetail> pbls = apbd.getOtherIncomeDetailByOiid(id);
			ArrayList als = new ArrayList();		
			long orders = 1l;
			double totalsum=0.00;
			for (OtherIncomeDetail ttd :  pbls) {
