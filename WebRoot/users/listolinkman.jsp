...
function addNew(){
	popWin("../users/toAddOlinkmanAction.do?OID=${OID}",900,650);
}

	function Update(){
		if(checkid>0){
			popWin("toUpdOLinkManAction.do?id="+checkid,900,650);
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	...
	function Del(){
		...
		if(window.confirm("您确认要删除编号为 "+checkid+" 的记录吗？如果删除将永远不能恢复!")){
			popWin2("../users/delOLinkmanAction.do?id="+checkid);
			}
		}else{
		alert("请选择你要操作的记录!");
		}
...
<form name="search" method="post" action="../users/listOlinkmanAction.do">
