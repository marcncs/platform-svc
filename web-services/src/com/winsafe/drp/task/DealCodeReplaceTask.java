...
public class DealCodeReplaceTask {

	// 同步锁
	private static Object lock = new Object();
	private static boolean isRunning = false;

	private static Logger logger = Logger.getLogger(DealCodeReplaceTask.class);

	private static List<UploadProduceLog> uploadPrLogs;
	private static List<UploadCartonSeqLog> uploadCsLogs;
	...
	public void init() throws Exception {
		uploadPrLogs = appLog.getAllUnProcessedLog(ProduceFileType.TOLLING_REPLACE.getValue());
		uploadCsLogs = appCartonSeqDao.getAllUnProcessedLog();
	}
		...
		if (!isRunning) {
			synchronized (lock) {
				try {
					...
					if(uploadCsLogs!=null&&uploadCsLogs.size()>0) {
						for(UploadCartonSeqLog log : uploadCsLogs) {
							execute(log);
						}
					}
	...
	private void execute(UploadCartonSeqLog log) throws Exception {
		DealCartonSeqService services = new DealCartonSeqService();
		services.dealCartonSeqFile(log, log.getPlNo());
	}
	
	public static void main(String[] args) {
		new DealCodeReplaceTask().run();
	}
}
