...
public class ExcPutOrganTradesProductTotalAction extends BaseAction{
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try{
	        ...
	        List list = asod.getOrganTradesProductTotal(whereSql);
			...
			response.setHeader("content-disposition",
					"attachment; filename=OrganTradesProductTotal.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出渠道换货按产品汇总");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "渠道换货按产品汇总",wchT));
			sheets[j].addCell(new Label(0, start+1, "换货机构:",seachT));
			sheets[j].addCell(new Label(1, start+1, request.getParameter("oname")));
			sheets[j].addCell(new Label(2, start+1, "产品名称:", seachT));
			sheets[j].addCell(new Label(3, start+1, request.getParameter("ProductName")));
			...
			sheets[j].addCell(new Label(0, start+4, "换货机构编号", wcfFC));
			sheets[j].addCell(new Label(1, start+4, "换货机构", wcfFC));
			sheets[j].addCell(new Label(2, start+4, "商品编号", wcfFC));
			sheets[j].addCell(new Label(3, start+4, "商品名称", wcfFC));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Label(1, row, organser.getOrganName(p.get("makeorganid").toString())));
				sheets[j].addCell(new Label(2, row, p.get("productid").toString()));
				sheets[j].addCell(new Label(3, row, setNull(p.get("productname"))));
				sheets[j].addCell(new Label(4, row, setNull(p.get("specmode"))));
