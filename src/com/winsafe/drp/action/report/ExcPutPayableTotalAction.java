...
public class ExcPutPayableTotalAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			AppPayable aso = new AppPayable();
			List list = aso.getPayableTotel(whereSql);
			...
			response.setHeader("content-disposition",
					"attachment; filename=PayableTotal.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出应付款结算单汇总");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "应付款结算单汇总",wchT));
			...
			sheets[j].addCell(new Label(5, start+4, "应付款描述",wcfFC));
			sheets[j].addCell(new Label(6, start+4, "应付款金额",wcfFC));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Label(5, row, p.get("payabledescribe").toString()));
				sheets[j].addCell(new Number(6, row, Double.valueOf(p.get("payablesum").toString()),wcfN));
				sheets[j].addCell(new Number(7, row, Double.valueOf(p.get("alreadysum").toString()),wcfN));
				totalrp += Double.valueOf(p.get("payablesum").toString());
				totalap += Double.valueOf(p.get("alreadysum").toString());
			}
			...
			sheets[j].addCell(new Number(6, row + 1, totalrp,wcfN));
			sheets[j].addCell(new Number(7, row + 1, totalap,wcfN));
