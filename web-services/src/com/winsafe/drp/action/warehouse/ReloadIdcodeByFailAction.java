...
public class ReloadIdcodeByFailAction extends BaseAction {
		...
		try {
			...
			if (idcodefile != null && !idcodefile.equals("")) {
				...
				if (extName != null) {
					...
					if (billsort == 18 || billsort == 19 || billsort == 21) {
						billsort = 1;
					}
			...
			if (!iuId.equals("")) {
				...
				if (!StringUtil.isEmpty(username)) {
					...
					iu.setMakeorganid(u.getMakeorganid());
					iu.setMakedeptid(u.getMakedeptid());
					iu.setMakeid(u.getUserid());
				...
				iu.setIsupload(0);
				iu.setUpusername(username);
				iu.setPhysicalpath(realPath);
				iu.setIsticket(2); //0为有单1为无单2为手工上传数据
				...
				if (flag) {
					...
					request.setAttribute("result", "文件已上传，正在处理中，条码上传详细信息请查看条码上传日志");
