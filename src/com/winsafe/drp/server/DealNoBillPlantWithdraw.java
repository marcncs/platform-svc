...
* @version:
 */
public class DealNoBillPlantWithdraw extends DealNoBillUploadService{
	
	private static Logger logger = Logger.getLogger(DealNoBillPlantWithdraw.class);
	
	public DealNoBillPlantWithdraw(){}

	public DealNoBillPlantWithdraw(String filepath, String fileaddress, int iuid,String username)
	{
	...
	public ArrayList<String> deal() throws Exception {
		try {
			...
			//处理上传的文件
			dealTxtFile(idcodeBeanList,Constants.SCAN_TYPE_PW_NOBILL);
			//缓存相关数据
			cacheData();
			//对上传后的产品进行分类整理
	...
	private void cacheData() throws Exception{
		...
		//工厂仓库权限
		plantWMap = UploadIdcodeUtil.getPlantWarehouse();
		//封装产品相关的Map信息
	...
	private void dealResultMap(Map<String,Map<String,DetailBean>> productDetailMap,Map<String,List<UploadIdcodeBean>> idcodeMap) throws Exception {
		for(String out_in_whid : idcodeMap.keySet()){
			...
			try {
				...
				//验证入库仓库权限
				boolean hasPlantAuth = validatePlantAuthority(inwarehouseid);
				...
				if(isOutExist && isInExist){
					...
					for(UploadIdcodeBean bean: idcodeMap.get(out_in_whid)){
						//判断权限
						boolean isAuth = checkAuthority(bean, isOutExist, hasRUWAuth, isInExist, hasPlantAuth);
						if(!isAuth){
							continue;
						}
						//验证条码重复
					...
					if(ttiList != null && ttiList.size() >0){
						...
						//自动签收单据
						autoReceiveBill(inwarehouseid,ttiList,productCountMap,idcodeCountMap,ttid,MEMO_PW_RECEIVE);
						//更改条码信息
 ...
 */
	...
	* @CreateTime Jan 10, 2013 4:43:46 PM
	...
	private String generateBill(String outwarehouseid,String inwarehouseid) throws Exception {
		...
		String owid =  MakeCode.getExcIDByRandomTableName("plant_withdraw", 2, "PW");
		ow.setId(owid);
