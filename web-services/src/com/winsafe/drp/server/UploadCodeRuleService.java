...
public class UploadCodeRuleService
{
	private Logger logger = Logger.getLogger(UploadCodeRuleService.class);
	//  单据号
	...
	// 产品单位
	private final static String P_UNIT_ID = "pUnitId";
	// 产品总数量
	...
	// 获取无单上传对像
	public UploadIdcodeBean getNoBillSamUploadIdcode(JSONObject jsonObject) throws Exception{
		...
		// 产品单位
		bean.setpUnitId(getPunitId(jsonObject));
		// 产品总数量
		bean.setSum(getSum(jsonObject));
		//发货类型
		if(jsonObject.has("deliveryType")
				&& !StringUtil.isEmpty(jsonObject.getString("deliveryType"))) {
			bean.setDeliveryType(jsonObject.getString("deliveryType"));
		}
		return bean;
	}
	...
	//获取产品单位
	public Integer getPunitId(JSONObject jsonObject) throws IdcodeException
	{
		return getIntegerProperty(jsonObject,P_UNIT_ID,true);
	}
	
	//获取产品总数量
	public Double getSum(JSONObject jsonObject) throws IdcodeException
	{
		return getDoublDateProperty(jsonObject,SUM,true);
	}
	...
	public Double getDoublDateProperty(JSONObject jsonObject,String property,boolean validate) throws IdcodeException
	{
		...
		try {
			...
			if(validate){
				throw new IdcodeException(UploadErrorMsg.getError(UploadErrorMsg.E00107, property));
			}
		}
		return value;
	}
	
	public Integer getIntegerProperty(JSONObject jsonObject,String property,boolean validate) throws IdcodeException
	{
		Integer value = null;
		try {
			String result = jsonObject.getString(property);
