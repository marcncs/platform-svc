...
var actid=0;
	...
	function Approve(){
		if(checkid!=""){	
			location.href("../finance/toApproveIncomeLogAction.do?ID="+checkid+"&actid="+actid);
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	...
	function CancelApprove(){
		if(checkid!=""){
			window.open("../finance/cancelApproveIncomeLogAction.do?ILID="+checkid+"&actid="+actid,"newwindow","height=250,width=500,top=250,left=250,toolbar=no,menubar=no,scrollbars=auto,resizable=no,location=no,status=no");
		}else{
		alert("请选择你要操作的记录!");
		}
	}
    ...
    <td><table width="100%" height="40" border="0" cellpadding="0" cellspacing="0" class="title-back">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="1">
          ...
          <tr align="center" class="table-back" onClick="CheckedObj(this,'${wd.id}','${wd.actid}');"> 
            <td ><a href="../finance/incomeLogDetailAction.do?ID=${wd.id}">${wd.id}</a></td>
            <td>${wd.drawee}</td>
            <td>${wd.incomesum}</td>
            <td>${wd.fundsrcname}</td>
            <td>${wd.fundattachname}</td>
            <td>${wd.actidname}</td>
...
<table  border="0" cellpadding="0" cellspacing="0">
          ...
          <td width="59%" align="right"> <presentation:pagination target="/finance/waitIncomeLogAction.do"/></td>
        </tr>
      </table>      </td>
  </tr>
</table>
