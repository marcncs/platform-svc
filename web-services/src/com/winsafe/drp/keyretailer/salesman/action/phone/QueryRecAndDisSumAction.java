...
public class QueryRecAndDisSumAction extends BaseAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if("1".equals(type)) {
				dateList = appTakeTicket.getReceiveSummary(request, loginUsers.getMakeorganid(), condition);
			} else if("2".equals(type)) {
				dateList = appTakeTicket.getDispatchSummary(request, loginUsers.getMakeorganid(), condition);
			} else if("3".equals(type)) {
				dateList = appTakeTicket.getReceiveSummaryForBKR(request, loginUsers.getMakeorganid(), condition);
			}
	...
	public static void main(String[] args) {
		System.out.println(Double.valueOf(".8"));
	}
}
