...
public class AppOtherIncomeAll {

	private static final Logger log = Logger.getLogger(AppOtherIncomeAll.class);

	public List getOtherIncomeAll(HttpServletRequest request, int pagesize,
			String pWhereClause) throws Exception {
		String hql = " from OtherIncomeAll  " + pWhereClause
				+ " order by makedate desc ";
		return PageQuery.hbmQuery(request, hql, pagesize);
	}
	
	public List getOtherIncomeAllList(HttpServletRequest request, int pagesize,
			String pWhereClause) throws Exception {
		String sql = "select oia.* from OTHER_INCOME_ALL oia " +
				"join organ o on o.id = oia.ORGANID " + pWhereClause;
		if(pagesize > 0){
			return PageQuery.jdbcSqlserverQuery(request, "makedate desc", sql, pagesize);
		} else {
			sql+=" order by oia.makedate desc ";
	...
	public double getTotalSum(String pWhereClause) throws Exception {
		String sql = "select sum(oid.subsum) from OtherIncomeAll as oi, OtherIncomeDetailAll as oid "
				+ pWhereClause + " and oi.id=oid.oiid";
		return EntityManager.getdoubleSum(sql);
	}

	public void addOtherIncomeAll(OtherIncomeAll otherincome) throws Exception {
		EntityManager.save(otherincome);
	}

	public void updOtherIncomeAll(OtherIncomeAll otherincome) throws Exception {
		EntityManager.save(otherincome);
	}

	public void delOtherIncomeAll(String id) throws Exception {
		String sql = "delete from Other_Income_all where id='" + id + "'";
		EntityManager.updateOrdelete(sql);

	}

	public List getOtherIncomeAllByBillno(String billno) throws Exception {
		String sql = "from OtherIncomeAll where billno='" + billno + "'";
		return EntityManager.getAllByHql(sql);
	}

	public OtherIncomeAll getOtherIncomeByID(String id) throws Exception {
		String sql = " from OtherIncomeAll where id='" + id + "'";
		return (OtherIncomeAll) EntityManager.find(sql);
	}

	public String getOtherIncomeNccodeByID(String id) throws Exception {
		String result = "";
		OtherIncomeAll otherIncome = new OtherIncomeAll();
	...
	public void updIsAudit(String ppid, int userid, int audit) throws Exception {
		String sql = "update other_income_all set isaudit=" + audit + ",auditid="
	...
	public void updIsNotAudit(String ppid) throws Exception {
		String sql = "update other_income_all set isaudit=0,auditid=null" +
				",auditdate=null where id='" + ppid + "'";
		EntityManager.updateOrdelete(sql);

	}

	public List<OtherIncomeAll> getOtherIncomeAll(String whereSql) {
	 ...
	 * @param nccode用友给出的编号
	 */
	public void delOtherIncomeAllByNCcode(String nccode) throws Exception {

		String sql = "delete from other_income_all where NCcode='" + nccode + "'";
		EntityManager.updateOrdelete(sql);
	...
	public boolean checkIsAudit(String nccode) {
		...
		String sql = " from OtherIncomeAll where NCcode='" + nccode + "'";
		OtherIncomeAll temp = new OtherIncomeAll();
		temp = (OtherIncomeAll) EntityManager.find(sql);
	 ...
	 */
	public static ReplyInfo saveOtherIncomeAll(OtherIncomeAll otherIncome) {
		ReplyInfo replyInfo = new ReplyInfo();
		boolean flag = false;
		String info = "";
		AppOtherIncomeAll appOtherIncome = new AppOtherIncomeAll();
		try {
			HibernateUtil.currentTransaction();
			appOtherIncome.addOtherIncomeAll(otherIncome);
			...
			e.printStackTrace();
			info = e.toString();
			log.error("------OtherIncomeAll保存------" + e.toString());
		}
	...
	public static ReplyInfo saveOtherIncomeDetail(
			OtherIncomeDetailAll otherIncomeDetail) {
		ReplyInfo replyInfo = new ReplyInfo();
		boolean flag = false;
		String info = "";
		AppOtherIncomeDetailAll appOtherIncomeDetail = new AppOtherIncomeDetailAll();
		try {
			HibernateUtil.currentTransaction();
	 ...
	 * @return result检货结果
	 */
	public static String auditOtherIncomeAll(String oiid, String userId) {
	...
	private static void addIdcode(OtherIncomeAll pi,
	...
	public OtherIncomeAll getOtherIncomeAllByNCcode(String nccode)
			throws Exception {
		String sql = " from OtherIncomeAll where NCcode='" + nccode + "'";
		return (OtherIncomeAll) EntityManager.find(sql);
	}
	...
	public ReplyInfo updateblankoutByNCcode(String nccode, String psnname)
		...
		String info = "";
		OtherIncomeAll temp = new OtherIncomeAll();
		temp = this.getOtherIncomeAllByNCcode(nccode);
		// 如果查不到该条记录则返回报错xml
		if (temp == null) {
			flag = false;
			...
			if (this.checkIsAudit(nccode)) {
				...
				this.delOtherIncomeAllByNCcode(nccode);
				AppOtherIncomeIdcode aoidcode = new AppOtherIncomeIdcode();
				AppOtherIncomeDetail aoid=new AppOtherIncomeDetail();
				aoidcode.delOtherIncomeIDcodeByNCcode(nccode);
				aoid.delOtherIncomeDetailByNCcode(nccode);
