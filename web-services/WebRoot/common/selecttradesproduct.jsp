...
function addObjValue(obj){
	for(g=0; g<parent.parray.length; g++){
		...
		if ( p.productid == obj.productid && p.warehouseid==obj.warehouseid){
			return;
		}
	}
...
function delObjValue(pid){
	for(g=0; g<parent.parray.length; g++){
		var p=parent.parray[g];
		if ( p.productid == pid.productid && p.warehouseid==pid.warehouseid){
	...
	if ( pid.length > 1 ){
		for ( i=0;i < pid.length; i++){
			for (j=0; j<parent.parray.length; j++){
				...
				if ( p.productid == pid[i].value && p.warehouseid==pobj[i].warehouseid){
					pid[i].checked = true;
				}				
			}
		}		
		...
		for (j=0; j<parent.parray.length; j++){
			var p=parent.parray[j];
			if ( p.productid == pid.value && p.warehouseid==pobj.warehouseid){
				pid.checked = true;
			}		
		}
	}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#BFC0C1">
  <tr>
	...
	<table width="100%" height="40" border="0" cellpadding="0" cellspacing="0" class="title-back">
       ...
       <form name="search" method="post" action="../common/selectTradesProductAction.do">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr class="title-func-back">
          ...
          <td width="1"><img src="../images/CN/hline.gif" width="2" height="14"></td>
          <td class="SeparatePage"><pages:pager action="../common/selectTradesProductAction.do"/></td>
        </tr>
      </table>
	  </div>
	</td>
	...
	<tr>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="1" class="sortable" id="ptable">
		  ...
		  <tr class="table-back-colorbar" > 
            ...
            <td>${p.productname}</td>
            <td>${p.specmode}</td>
            <td><input type="text" name="countunit" size="6" value="${p.unitidname}" onFocus="selectUnit(this,'${p.productid}','${cid}','${organid}','2')"></td>
			<td>${p.warehouseidname}</td>
			<td><span id="spanstockpile${count}">${p.quantity}</span>
            <input type="hidden" name="stockpile" id="stockpile${count}" value="${p.stockpile}">
            <input type="hidden" name="pobj" id="pobj${count}" value="{productid:'${p.productid}',productname:'${p.productname}',specmode:'${p.specmode}',unitid:'${p.unitid}',unitidname:'${p.unitidname}',warehouseid:'${p.warehouseid}',warehouseidname:'${p.warehouseidname}'}">
            </td>
			</tr>
