...
document.writeln('<iframe id=showsqLayer scrolling="no" Author=jelli frameborder=0 style="position: absolute; width: 200; height:145;  display:none ;z-index:999;"></iframe>');
strFrame='<style>';
strFrame+='.seldiv{position:absolute;z-index:1000;overflow-x:auto;overflow-y:auto;';
strFrame+='left:0; top:0;background-color:#ffffcc;width:200;height:145;';
...
strFrame+='<tr class="title-back"><td width="20%" height="30"> 仓库</td><td width="20%">仓位</td><td width="30%"> 库存量</td></tr>';
...
var outButton;  //点击的按钮
var billtype;
function ShowWB(v_tt,v_productid,v_warehouseid,v_stockpile) //主调函数
{
 ...
 //-------------------------------
 getStockByOid(v_productid,v_warehouseid,v_stockpile);
 //--------------------------------
...
function getStockByOid(v_productid,v_warehouseid,v_stockpile){
//	var url = '../purchase/getPurchaseHistoryChenAjax.do?pid='+v_pid+"&productid="+v_productid;
	var url = '../warehouse/getWarehouseBitAjax.do?productid='+v_productid+"&warehouseid="+v_warehouseid+"&stockpile="+v_stockpile;
	var pars = '';
...
function showprice(originalRequest){
	...
	if ( lk == undefined ){
		...
		for(var j=0; j<lk.length; j++){
			sst+='<tr><td width="20%">'+lk[j].wname+'</td><td width="20%" align="center">'+lk[j].warehouseBitName+' </td><td width="30%" align="left">'+lk[j].stockpile+' </td></tr>'; 
		}
//		sst+='<tr><td width="50%" align="right">合计：</td><td width="50%" align="center">'+data.total+'</td></tr>';
		sst+='</table>';
