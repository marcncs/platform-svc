...
public class ExcPutWithdrawProductTotalAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			response.setHeader("content-disposition",
					"attachment; filename=WithdrawProductTotal.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出零售退货按产品汇总");	
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "零售退货按产品汇总",wchT));
			...
			sheets[j].addCell(new Label(2, start+3, "产品编号",wcfFC));
			sheets[j].addCell(new Label(3, start+3, "产品名称",wcfFC));
			sheets[j].addCell(new Label(4, start+3, "规格",wcfFC));
			sheets[j].addCell(new Label(5, start+3, "单位",wcfFC));
			sheets[j].addCell(new Label(6, start+3, "数量",wcfFC));
			sheets[j].addCell(new Label(7, start+3, "金额",wcfFC));
			int row = 0;
			Double totalqt = 0.00;
