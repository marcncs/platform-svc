...
public class ExcPutSaleTradesDetailAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			response.setHeader("content-disposition",
					"attachment; filename=SaleTradesDetail.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出零售换货明细");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "零售换货明细",wchT));
			...
			sheets[j].addCell(new Label(8, start+4, "批次",wcfFC));
			...
			for (int i = start; i < currentnum; i++) {
				...
				String organName = organService.getOrganName(p.get("makeorganid").toString());
				sheets[j].addCell(new Label(4, row, organName));
				...
				sheets[j].addCell(new Label(8, row, p.get("batch")==null?"":p.get("batch").toString()));
			...
			sheets[j].addCell(new Number(9, row + 1, totalsum,QWCF));
