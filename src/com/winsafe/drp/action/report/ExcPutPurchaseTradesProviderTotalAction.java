...
public class ExcPutPurchaseTradesProviderTotalAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			response.setHeader("content-disposition",
					"attachment; filename=PurchaseTradesProviderTotal.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出采购换货按供应商汇总");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os, HttpServletRequest request)
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "采购换货按供应商汇总",wchT));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Label(0, row, p.get("provideid")
						.toString()));
				sheets[j].addCell(new Label(1, row, p.get("providename")
						.toString()));
				sheets[j].addCell(new Label(2, row, p.get("productid")
						.toString()));
				sheets[j].addCell(new Label(3, row, p.get("productname")
						.toString()));
			...
			sheets[j].addCell(new Number(4, row + 1, totalsum,QWCF));
