...
function Check(){
    ...
    function closePlan(){
   		...
   		if(rnum!=''&& rnum!='0'){
   			if(parseInt(pnum,10) == parseInt(rnum,10) + parseInt(lastnum,10)){
  				if (confirm("请确认:计划生产"+pnum+"箱,释放"+rnum+"箱,实际生产"+lastnum+"箱")){
						popWin2("../erp/closeProductPlanAction.do?type=reProcess&ID="+pid);
						 var bb = setTimeout("winClose(1);",500);
					}
			}else if(parseInt(rnum,10)<parseInt(pnum,10)-parseInt(lastnum,10)){
				alert("计划生产"+pnum+"箱,实际生产"+lastnum+"箱,您还要释放"+(parseInt(pnum,10)-parseInt(lastnum,10)-parseInt(rnum,10))+"箱条码!");
				return;
			}
			else if(parseInt(rnum,10)>parseInt(pnum,10)-parseInt(lastnum,10)){
				alert("计划生产"+pnum+"箱,实际生产"+lastnum+"箱,请取消释放"+(parseInt(lastnum,10)+parseInt(rnum,10)-parseInt(pnum,10))+"箱条码!");
				return;
			}
   		}else{
   			if(parseInt(pnum,10)==parseInt(lastnum,10)){
   				...
   				alert("计划生产"+pnum+"箱,实际生产"+lastnum+"箱,您还要释放"+(parseInt(pnum,10)-parseInt(lastnum,10))+"箱条码!");
				return;
   			}
   		}
...
<body>
<% 
String pid = (String)request.getAttribute("productPlanId"); 
request.getSession().setAttribute("productPlanId",pid);
%>
<style>
...
<!--	      <windrp:getname key='YesOrNo' value='${plan.approvalFlag}' p='f'/>-->
	  ...
	  <td align="right">
		...
		<td>
		   ...
		   maxlength="8" onkeyup="checknum(this)" onblur="checknum(this)"
		    dataType="Require" msg="必须录入实际生产箱数!">
		    <span class="style1">*</span>
		</td>
		<td>
		<input type="button" name="button" value="确认结束" onClick="javascript:closePlan();">
		</td>
		</tr>
	  </table> 
	  ...
	  <br>
		  ...
		  <tr>
		    <td>
			<div id="bodydiv">
			<form name="search" method="post" action="../erp/listProductPlanDetailAction.do?close=1">
			  ...
			  <table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr class="title-func-back">
				 <!-- <td width="50" >
					...
					<td width="50" >
					<ws:hasAuth operationName="/erp/releaseCodeAction.do">
					<c:choose>
					  <c:when test="${ (( empty plan.closeFlag) || plan.closeFlag != 1 )&&printflag==1}">
					  	<a href="javascript:Del();"><img src="../images/CN/setmima.gif" width="16" height="16" border="0" align="absmiddle">&nbsp;释放</a>
					  </c:when>
					  <c:otherwise>
					  	<a href="#" disabled><img src="../images/CN/setmima.gif" width="16" height="16" border="0" align="absmiddle">&nbsp;释放</a>
					  </c:otherwise>
		          	</c:choose>	
		          	</ws:hasAuth>		
					</td>
					...
					<c:choose>
					  <c:when test="${ (( empty plan.closeFlag) || plan.closeFlag != 1 )&&printflag==1}">
					  	<a href="javascript:Del2();"><img src="../images/CN/setmima.gif" width="16" height="16" border="0" align="absmiddle">&nbsp;取消释放</a>
					  </c:when>
					  <c:otherwise>
					  	<a href="#" disabled><img src="../images/CN/setmima.gif" width="16" height="16" border="0" align="absmiddle">&nbsp;取消释放</a>
					  </c:otherwise>
		          	</c:choose>	
		          	</ws:hasAuth>		
					</td>
				  ...
				  <pages:pager action="../erp/listProductPlanDetailAction.do?close=1"/></td>						
				</tr>
			</table>	
			 </div>
			</td>
