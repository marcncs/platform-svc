...
public class ExcPutReceivableTotalAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			AppReceivable aso = new AppReceivable();
			List list = aso.getReceivableTotal(whereSql);
			...
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出应收款结算单汇总");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "应收款结算单汇总",wchT));
			...
			sheets[j].addCell(new Label(5, start+4, "应收款描述",wcfFC));
			sheets[j].addCell(new Label(6, start+4, "应收款金额",wcfFC));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Label(1, row, p.get("roid").toString()));
				ReceivableObject ro = (ReceivableObject)aso.getReceivableObjectByIDOrgID(p.get("roid").toString(),p.get("makeorganid").toString());
				...
				sheets[j].addCell(new Label(5, row, p.get("receivabledescribe").toString()));
				sheets[j].addCell(new Number(6, row, Double.valueOf(p.get("receivablesum").toString()),wcfN));
				sheets[j].addCell(new Number(7, row, Double.valueOf(p.get("alreadysum").toString()),wcfN));
				totalrv += Double.valueOf(p.get("receivablesum").toString());
				totalav += Double.valueOf(p.get("alreadysum").toString());
			}
			...
			sheets[j].addCell(new Label(5, row + 1, "合计："));
			sheets[j].addCell(new Number(6, row + 1, totalrv,wcfN));
			sheets[j].addCell(new Number(7, row + 1, totalav,wcfN));
