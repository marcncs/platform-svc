...
public class ImportOtherOutcomeProductAction extends BaseAction {
	private Logger logger = Logger.getLogger(ImportOtherOutcomeProductAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		// 数据用对象保存
		List<OtherShipmentBillDAllForm> list = new ArrayList<OtherShipmentBillDAllForm>();
		try {
			...
			if (bool) {
				...
				for (int i = 1; i < row; i++) {
					...
					try {
						...
						// 打造数据
						OtherShipmentBillDAllForm otherShipmentBillDAllForm = new OtherShipmentBillDAllForm();
						otherShipmentBillDAllForm.setProductid(productid);
						otherShipmentBillDAllForm.setProductname(product.getProductname());
						otherShipmentBillDAllForm.setSpecmode(product.getSpecmode());
						otherShipmentBillDAllForm.setBatch(batch);
						otherShipmentBillDAllForm.setQuantity(Double.valueOf(quantity));
						otherShipmentBillDAllForm.setCountUnit(Constants.DEFAULT_UNIT_ID);
...
//						
						list.add(otherShipmentBillDAllForm);
						CCount++;
					} catch (Exception e) {
				...
				try {
					...
					if (list.size() > 0) {
						...
						AddOtherOutcomeService addOtherOutcomeService = new AddOtherOutcomeService();
						addOtherOutcomeService.addOtherOutcome(organId, warehouseId, incomeSort, billNo, remark, users, isAccount, list);
