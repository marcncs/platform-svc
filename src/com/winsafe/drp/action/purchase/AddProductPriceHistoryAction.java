...
public class AddProductPriceHistoryAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			//判断开始日期与结束日期是否有重叠部分
			boolean isFlag = apph.getProductPriceHistoryByProductIdAndTime(productid, starttime, endtime);
			//如果有重叠部分返回
			if(isFlag){
			...
			apph.addProductPriceHistory(pph);
			
			request.setAttribute("result", "databases.add.success");
			DBUserLog.addUserLog(userid, 11, "新增商品历史价格");
