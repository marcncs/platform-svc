...
public class ExcPutWithdrawDetailAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			response.setHeader("content-disposition",
					"attachment; filename=WithdrawDetail.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出零售退货明细");	
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "零售退货明细",wchT));
			...
			sheets[j].addCell(new Label(0, start+4, "客户",wcfFC));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Label(0, row, p.get("cname").toString()));
				sheets[j].addCell(new Label(1, row, p.get("id").toString()));
				sheets[j].addCell(new Label(2, row, p.get("makedate").toString()));
				
				String organName = organService.getOrganName(p.get("makeorganid").toString());
				sheets[j].addCell(new Label(3, row, organName));
				...
				sheets[j].addCell(new Number(7, row, Double.valueOf(p.get("unitprice").toString()),wcfN));
				sheets[j].addCell(new Number(8, row, Double.valueOf(p.get("quantity").toString()),QWCF));
				sheets[j].addCell(new Number(9, row, Double.valueOf(p.get("subsum").toString()),wcfN));
