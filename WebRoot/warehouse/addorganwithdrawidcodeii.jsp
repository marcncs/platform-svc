...
function addIdcode(){
	...
	var batch=$F('batch');	
   var url = "../warehouse/addOrganWithdrawIdcodeiiAction.do?idcode="+idcode+"&billid="+billid+"&prid="+prid+"&warehousebit="+warehousebit+"&batch="+batch;
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#BFC0C1" >
  <tr>
	...
	<tr>
          ...
          <td width="9%">${bbd.quantity}</td>
	       
	  </tr>
	  <tr><td colspan="8"></tr>
	  <tr>
	  	<td align="right">条码：</td>
          <td><input type="text" name="idcode" size="35" maxlength="50"></td>
          <td align="right">仓库：</td>
	      <td><windrp:getname key='warehouse' value='${wid}' p='d'/></td>		
          <td align="right">仓位：</td>
          <td><select name="warehousebit">
            <logic:iterate id="w" name="bitlist">
              <option value="${w.wbid}">${w.wbid}</option>
            </logic:iterate>
          </select></td>
<!--	      <td width="15%" ><input type="button" value="新增" onClick="formcheck()" ${isaudit==1?"disabled":""} disabled>&nbsp;<input type="button" name="cancel" value="取消" onClick="window.close();"></td>          -->
	  </tr>
      ...
      name="submsg" src="../warehouse/listOrganWithdrawIdcode.do?billid=${billid}&prid=${bbd.productid}&isaudit=${isaudit}&flag=${flag}" frameBorder="0" scrolling="no" onload="setIframeHeight(this)"></IFRAME>  
