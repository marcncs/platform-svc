...
public class QueryRecAndDisSumDetailAction extends BaseAction {
	private Logger logger = Logger.getLogger(QueryRecAndDisSumDetailAction.class);
...
//	Map<String, String> unitMap = null;
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if("1".equals(type)) {
				dateList = appTakeTicket.getReceiveSummaryDetail(request, loginUsers.getMakeorganid(), condition);
			} else if("2".equals(type)) {
				dateList = appTakeTicket.getDispatchSummaryDetail(request, loginUsers.getMakeorganid(), condition);
			} else if("3".equals(type)) {
				dateList = appTakeTicket.getReceiveSummaryDetailForBKR(request, loginUsers.getMakeorganid(), condition);
			}
