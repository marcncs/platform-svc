...
var checkid="";
	...
	function addNew(){
		popWin("toAddSuitAction.do?objsort="+${objsort},1000,600);
	}

	function Update(){
		if(checkid!=""){
			popWin("toUpdSuitAction.do?id="+checkid,1000,600);
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	...
	function Del(){
		if(checkid!=""){
		if(window.confirm("您确认要删除编号为："+checkid+" 的抱怨投诉记录吗？如果删除将永远不能恢复!")){
			popWin2("delSuitAction.do?ID="+checkid);
			}
		}else{
		alert("请选择你要操作的记录!");
		}
	...
	function Detail(){
	if(checkid!=""){
		document.all.submsg.src="suitDetailAction.do?id="+checkid;
		}else{
		alert("请选择你要操作的记录!");
		}
	}
...
<form name="search" method="post" action="listSuitAllAction.do">
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							...
							<tr class="table-back">
								<td align="right">
									制单机构：
									<input type="hidden" id="objsort" name="objsort" value="${objsort}"/>								</td>
							  <td>
...
<input type="text" name="uname" id="uname" onClick="selectDUW(this,'MakeID',$F('MakeOrganID'),'ou')" value="${uname}" readonly>								</td>
							...
							<tr class="table-back">
								<td align="right">
									投诉种类：								</td>
								<td>
									<windrp:select key="SuitContent" name="SuitContent" p="y|d" value="${SuitContent}"/>								</td>
								<td align="right">
									投诉方式：								</td>
								<td>
									<windrp:select key="SuitWay" name="SuitWay" p="y|d" value="${SuitWay}"/>								</td>
								<td align="right">
									是否处理：								</td>
								<td>
									<windrp:select key="YesOrNo" name="IsDeal" p="y|f" value="${IsDeal}"/>								</td>
							</tr>
							<tr class="table-back">
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<tr class="title-func-back">
								...
								<td class="SeparatePage">
									<pages:pager action="../sales/listSuitAllAction.do" />
								</td>
							</tr>
						</table>
					</div>
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="1"
							...
							<logic:iterate id="p" name="hList">
								<tr class="table-back-colorbar"
									...
									<td>
									<windrp:getname key='SuitContent' value='${p.suitcontent}' p='d'/>
									</td>
									<td>
									<windrp:getname key='SuitWay' value='${p.suitway}' p='d'/>
									</td>
									<td>
									<windrp:getname key='YesOrNo' value='${p.isdeal}' p='f'/>
									</td>
									<td>
      	...
      	<div id="tabs1">
            <ul>
              <li><a href="javascript:Detail();"><span>抱怨投诉详情</span></a></li>
            </ul>
          </div>
