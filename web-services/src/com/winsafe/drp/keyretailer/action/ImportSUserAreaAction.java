...
public class ImportSUserAreaAction extends BaseAction {
	private static Logger logger = Logger.getLogger(ImportSUserAreaAction.class);
		...
		try {
			...
			if (bool) {
				...
				for (int i = 1; i < row; i++) {
					...
					// 获取用户编号
					String userId = sheet.getCell(1, i).getContents() == null ? null : sheet
							.getCell(1, i).getContents().trim();
					if (StringUtil.isEmpty(userId)) {
						errMsg.append("第[" + (i + 1) + "]行用户编号不能为空<br />");
						continue;
					}
					areaIds.append(","+areaId);
					userIds.append(","+userId);
					dataList.add(new String[]{String.valueOf(i + 1), areaId, userId});
				}
				
				if(areaIds.length() > 0) {
					List<String> sqls = new ArrayList<String>();
					Set<String> areaIdSet = appSBonusArea.getAreaIdSet(areaIds.substring(1));
					Set<String> userIdSet = appSUserArea.getUserIdSet(userIds.substring(1));
					for(String[] data : dataList) {
						...
						if(!userIdSet.contains(data[2])) {
							errMsg.append("第[" + data[0] + "]行用户编号"+data[2]+"在系统中不存在<br />");
							continue;
						}
						sqls.add("delete from S_USER_AREA where userId="+data[2]);
						sqls.add("INSERT INTO S_USER_AREA VALUES (S_USER_AREA_SEQ.nextval,"+data[2]+","+data[1]+",null,sysdate,0)");
						CCount++;
					}
					appSUserArea.updSUserAreaByBatch(sqls);
				}
				wb.close();
			} else {
