...
public class ExcPutProductIncomeTotalAction extends BaseAction{
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try{
	        ...
	        List list = asod.getTotalReport(whereSql);
			...
			response.setHeader("content-disposition",
					"attachment; filename=ProductIncomeTotal.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出产成品入库汇总");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "产成品入库汇总",wchT));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Number(5, row, Double.valueOf(p.get("costsum").toString()),wcfN));
