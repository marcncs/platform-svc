...
public class ExcPutSaleBillTotalAction extends BaseAction {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			response.setHeader("content-disposition",
					"attachment; filename=SaleBillTotal.xls");
			response.setContentType("application/msexcel");
			writeXls(list, os, request);
			os.flush();
			os.close();
			DBUserLog.addUserLog(userid, 10,"报表分析>>导出零售按单据汇总");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public void writeXls(List list, OutputStream os,
		...
		for (int j = 0; j < snum; j++) {
			...
			sheets[j].addCell(new Label(0, start, "零售按单据汇总",wchT));
			...
			sheets[j].addCell(new Label(2, start+4, "会员编号", wcfFC));
			sheets[j].addCell(new Label(3, start+4, "会员名称", wcfFC));
			sheets[j].addCell(new Label(4, start+4, "会员手机", wcfFC));
			sheets[j].addCell(new Label(5, start+4, "制单机构", wcfFC));
			sheets[j].addCell(new Label(6, start+4, "配送机构", wcfFC));
			...
			for (int i = start; i < currentnum; i++) {
				...
				sheets[j].addCell(new Label(6, row, p.get("equiporganid").toString()));
				sheets[j].addCell(new Number(7, row, Double.valueOf(p.get("totalsum").toString()),wcfN));
