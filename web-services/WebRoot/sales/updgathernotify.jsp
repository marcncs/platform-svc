...
function SupperSelect(rowx){
	...
	showModalDialog("toSelectProductAction.do",null,"dialogWidth:20cm;dialogHeight:16cm;center:yes;status:no;scrolling:no;");
...
function SelectCustomer(){
	var lk=showModalDialog("toSelectCustomerAction.do",null,"dialogWidth:16cm;dialogHeight:8.5cm;center:yes;status:no;scrolling:no;");
	document.validateProvide.cid.value=lk.cid;
	document.validateProvide.cname.value=lk.cname;
}
  ...
  <tr>
    <td><table width="100%" height="40" border="0" cellpadding="0" cellspacing="0" class="title-back">
        <tr> 
          ...
          <td width="772"> 修改打款通知</td>
        </tr>
      </table>
      <form name="validateProvide" method="post" action="../sales/updGatherNotifyAction.do" onSubmit="return ChkValue();">
	  ...
	  <tr>
	    ...
	    <td><select name="saleid">
            <logic:iterate id="w" name="als">
              <option value="${w.userid}" ${w.userid==sof.saleid?"selected":""}>${w.realname}</option>
            </logic:iterate>
          </select></td>
		<td  align="right">结算方式：</td>
	    <td>${sof.paymentmodename}</td>
	    <td align="right">入账银行账号：</td>
	    <td><input name="bankaccount" type="text" id="bankaccount" value="${sof.bankaccount}"></td>
	    </tr>
	  <tr>
	    
	    <td align="right">零售部门：</td>
	    <td><select name="saledept">
            <logic:iterate id="w" name="aldept">
              <option value="${w.id}" ${w.id==sof.saledept?"selected":""}>${w.deptname}</option>
            </logic:iterate>
          </select></td>
		<td  align="right">相关单据号：</td>
	    <td><input name="billno" type="text" id="billno" size="30" value="${sof.billno}"></td>
	    <td align="right">金额：</td>
	    <td><input name="totalsum" type="text" id="totalsum" size="20" value="${sof.totalsum}"></td>
	    </tr>
	  </table>
</fieldset>
       ...
       <tr>
            <td  align="center" ><table width="100%"  border="0" cellpadding="0" cellspacing="1">
              <tr >
                ...
                <td width="94%"><textarea name="memo" cols="180" rows="4" id="memo">${sof.memo}</textarea></td>
              </tr>
            </table></td>
          </tr>
          <tr>
