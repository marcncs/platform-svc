...
var checkid=0;
	...
	function addNew(){
	popWin("../warehouse/toAddHarmShipmentBillAction.do",1000,650);
	}

	function Update(){
		if(checkid!=""){
		popWin("toUpdHarmShipmentBillAction.do?ID="+checkid,1000,650);
		}else{
		alert("请选择你要操作的记录!");
		}
	}
	...
	function OtherShipmentDetail(){
		if(checkid!=""){
			document.all.submsg.src="harmShipmentBillDetailAction.do?ID="+checkid;
		}else{
			alert("请选择你要操作的记录!");
		}
	}
	...
	function excput(){
		search.target="";
		search.action="../warehouse/excPutHarmShipmentBillAction.do";
		search.submit();
	}
	function oncheck(){
		search.target="";
		search.action="../warehouse/listHarmShipmentBillAction.do";
		search.submit();
	}
	function print(){
		search.target="_blank";
		search.action="../warehouse/printListHarmShipmentBillAction.do";
		search.submit();
	}
	...
	function Del(){
		...
		if(window.confirm("您确认要删除编号为:"+checkid+"的记录吗？如果删除将永远不能恢复!")){
			popWin2("../warehouse/delHarmShipmentBillAction.do?OSID="+checkid);
			}
		}else{
		alert("请选择你要操作的记录!");
		}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#BFC0C1">
 ...
 <form name="search" method="post" action="../warehouse/listHarmShipmentBillAction.do" onSubmit="return oncheck();">
      <table width="100%"  border="0" cellpadding="0" cellspacing="0">
		...
		<tr class="table-back">
          ...
          <tr class="table-back"> 
            <td width="12%"  align="right">出货仓库：</td>
            <td width="22%"><input type="hidden" name="WarehouseID" id="WarehouseID" value="${WarehouseID}">
            ...
            <td width="22%"><windrp:select key="YesOrNo" name="IsAudit" p="y|f" value="${IsAudit}" /></td>
  ...
  <input name="EndDate" value="${EndDate}" type="text" onFocus="javascript:selectDate(this)" size="12" readonly></td>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr class="title-func-back">
          ...
          <td width="50">
			...
			<td width="1"><img src="../images/CN/hline.gif" width="2" height="14"></td>	
          <td style="text-align: right;"><pages:pager action="../warehouse/listHarmShipmentBillAction.do" />
          </td>
        </tr>
      </table>
	  </div>
	...
	<logic:iterate id="s" name="alsb" > 
      ...
      <div style="width:100%">
        <div id="tabs1">
          <ul>
            <li><a href="javascript:OtherShipmentDetail();"><span>报损出库详情</span></a></li>
          </ul>
        </div>
