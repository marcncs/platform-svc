	...
	function Affirm(){
		...
		for(var i=0;i<document.listform.pid.length;i++){
			if(document.listform.pid[i].checked){
				...
				arrpaymentmode[k]=document.listform.paymode[i].value;
				arrpaymentmodename[k]=document.listform.paymodename[i].value;
				arrbillno[k]=document.listform.billno[i].value;
				arrreceivablesum[k]=document.listform.payablesum[i].value;
				k++;
				...
				arrpaymentmode[0]=document.listform.paymode.value;
				arrpaymentmodename[0]=document.listform.paymodename.value;
				arrbillno[0]=document.listform.billno.value;
				arrreceivablesum[0]=document.listform.payablesum.value;
				flag=true;//只要选中一个就设为true
		}
		if(flag){
			
			var p={pid:arrid.slice(0),paymentmode:arrpaymentmode.slice(0),paymentmodename:arrpaymentmodename.slice(0),billno:arrbillno.slice(0),payablesum:arrreceivablesum.slice(0)};
    ...
    <td><table width="100%" height="40" border="0" cellpadding="0" cellspacing="0" class="title-back">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="1">
		  ...
		  <tr class="table-back" > 
            ...
            <td >${p.id}</td>
            <td><input type="hidden" name="paymode" value="${p.paymode}"><input type="hidden" name="paymodename" value="${p.paymodename}">${p.paymodename}</td>
            <td><input type="hidden" name="billno" value="${p.billno}">${p.billno}</td>
            <td><input type="hidden" name="payablesum" value="${p.payablesum}">
              ${p.payablesum}</td>
          </tr>
          </logic:iterate> 
