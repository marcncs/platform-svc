...
<script language=javascript>
	function CheckInput(){
		...
		var istime =referform.istime.value;
		if(tagsubvalue==null||tagsubvalue==""){
			alert("值不能为空!");
			return false;
		} else {
			if(istime == "true") {
				if(!checkTime(tagsubvalue)) {
					alert("日期格式不正确, 正确格式为 时:分 :秒");
					return false;
				}
			} else {
				var regNum = /^\d*$/;
				if(!regNum.test(tagsubvalue)) {
					alert("格式不正确, 必须为整数");
					return false;
				}
			}
...
function checkTime(time){
    ...
    if (regTime.test(time)) {
        ...
        if ((parseInt(array[0]) < 24) && (parseInt(array[1]) < 60) && (parseInt(array[2]) < 60)) {
            result = true;
        }
    }
    return result;
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#BFC0C1">
  <tr>
    <td><table width="100%" height="40" border="0" cellpadding="0" cellspacing="0" class="title-back">
	  ...
	  <input name="tagsubkey" type="hidden" id="tagsubkey" value="${br.tagsubkey}" readonly>
	  <input name="isuse" type="hidden" id="isuse" value="${br.isuse}" readonly>
	  <input name="istime" type="hidden" id="istime" value="${istime}" readonly>
