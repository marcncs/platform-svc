		...
		try {
			...
			if("1".equals(type)) {
				if(DealerType.BKD.getValue().equals(loginUsers.getOrganModel())) {
					dateList = appTakeTicket.getReceiveSummaryDetail(request, loginUsers.getMakeorganid());
				} else {
					dateList = appTakeTicket.getReceiveSummaryDetail(request, loginUsers.getMakeorganid());
				}
			} else {
				dateList = appTakeTicket.getDispatchSummaryDetail(request, loginUsers.getMakeorganid());
			}
